...
**This app has no set-password gate of its own.** Accounts it creates are
flagged `must_change_password`, and the *Launchpad* is what enforces it: the
person is sent to `/set-password` the first time they open the Launchpad and
cannot go further until they have replaced the temporary one. Someone who only
ever uses this app is never prompted, and keeps the password an admin issued
them. Adding a gate here would close that.
